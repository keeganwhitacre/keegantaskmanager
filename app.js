...
var CAT_DEFAULTS = { manuscript:'manuscript', lab:'lab ops', phd:'phd apps', conf:'conference', bel:'bel ♡', personal:'personal', hobby:'hobby' };
var CAT_LABEL = Object.assign({}, CAT_DEFAULTS);
...
var migrated = false;
try { migrated = localStorage.getItem('kw_migrated_v1') === '1'; } catch(e){}
if(!migrated){
...
try { localStorage.setItem('kw_migrated_v1', '1'); } catch(e){}
saveLocal();
}
...
var ghPushDebounceTimer = null;
var ghPushImmediate = false;
// Raw push — do not call directly, use ghPush() which debounces
function ghPushNow(){
...
// Debounced public entry point — all callers use this
...
ghPushDebounceTimer = setTimeout(function(){ ghPushDebounceTimer = null; ghPushNow(); }, 2500);
}
...
window.addEventListener('online', function(){ if(state.pendingSync) ghPush(true); });
...
// Rebuild filter row chips (keep All, Today, Blocked, Archive; replace cat chips)
var filterRow = document.getElementById('filterRow');
if(filterRow){
// Remove existing category chips (everything except fixed ones)
var fixed = ['all','today','blocked','archive'];
filterRow.querySelectorAll('.chip').forEach(function(c){ if(fixed.indexOf(c.dataset.filter) === -1) c.remove(); });
// Re-insert after 'today' chip
var todayChip = filterRow.querySelector('[data-filter="today"]');
var blockedChip = filterRow.querySelector('[data-filter="blocked"]');
...
// Rebuild category chip row in add/edit sheet
var catRow = document.getElementById('catRow');
if(catRow){
var activeVals = [];
catRow.querySelectorAll('.s-chip.active').forEach(function(c){ activeVals.push(c.dataset.val); });
catRow.innerHTML = '';
Object.keys(CAT_LABEL).forEach(function(key){
var c = document.createElement('div');
...
var container = document.getElementById('catSettingsRows');
if(!container) return;
container.innerHTML = '';
Object.keys(CAT_LABEL).forEach(function(key){
...
if(!container) return;
var newCats = {};
container.querySelectorAll('.cat-settings-row').forEach(function(row){
var key = row.querySelector('.cat-key-input').value.trim().toLowerCase().replace(/\s+/g,'_');
var label = row.querySelector('.cat-label-input').value.trim();
...
var saveCatBtn = document.getElementById('saveCatsBtn');
...
var container = document.getElementById('catSettingsRows');
if(!container) return;
var row = document.createElement('div'); row.className = 'cat-settings-row';
...
if(del){ var row = del.closest('.cat-settings-row'); if(row) row.remove(); }
});
...
// INIT
loadTheme(); loadLocal(); rebuildCategoryUI(); render(); loadSettingsUI();
